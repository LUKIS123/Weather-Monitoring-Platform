...
public interface IStationsPermissionsRepository
{
    ...
    Task AddPermissionRequestAsync(int stationId, string userId, PermissionStatus status, DateTime createdAt);
    ...
    Task<UsersPermissionDto?> GetUsersPermissionAsync(string userId, int deviceId);
}
